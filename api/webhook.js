...
function buildCalendarLinks(confirmedData, token) {
  const date = (confirmedData && confirmedData['Date']) || '';
  const time = (confirmedData && confirmedData['Time']) || '';
  if (!date) return null;
  ...
  const loc = (confirmedData && confirmedData['Location']) || '';
  const service = (confirmedData && confirmedData['Service']) || 'Makeup Appointment';
  const numPeople = (confirmedData && confirmedData['Number of People']) || '';
  ...
  const ics = `${SITE_URL}/api/calendar?${new URLSearchParams({ title, date, time: time || '09:00', location: loc, description: desc, uid: `${token}@theglambyankita.com` })}`;
      ...
      if (user && pass) {
        ...
        if (bookingToken) {
          ...
          if (result.rows[0] && result.rows[0].confirmed_data) {
            confirmedData = result.rows[0].confirmed_data;
            bookingDetailsRows = Object.entries(confirmedData)
              .filter(([, v]) => v)
        ...
        const calLinks = buildCalendarLinks(confirmedData, bookingToken || 'booking');
      ...
      <table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
        ...
        <tr>
          <td style="padding:24px 32px;">
            <p style="margin:0;font-size:0.88rem;color:#4a2e22;line-height:1.7;">The booking is now locked in. View all payments in your <a href="https://dashboard.stripe.com/payments" style="color:#c9a96e;">Stripe dashboard</a>.</p>
          </td>
        </tr>
        ...
        if (clientEmail) {
          const firstName = (confirmedData && confirmedData['First Name']) || clientName;
          const detailRows = confirmedData
            ? Object.entries(confirmedData)
                .filter(([, v]) => v)
                .map(([k, v]) => `<tr><td style="padding:9px 16px;font-size:0.82rem;font-weight:700;color:#9a7060;text-transform:uppercase;letter-spacing:0.07em;white-space:nowrap;width:38%;border-bottom:1px solid #fdeee8;">${k}</td><td style="padding:9px 16px;font-size:0.9rem;color:#2c1810;border-bottom:1px solid #fdeee8;">${v}</td></tr>`)
                .join('')
            : '';
...
<head>
  ...
  <title>Booking Confirmed — The Glam by Ankita</title>
</head>
<body style="margin:0;padding:0;background:#f5ede8;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5ede8;padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.07);">
        ...
        <tr>
          <td style="padding:28px 36px 0;">
            ...
            <p style="margin:10px 0 0;font-size:0.95rem;color:#4a2e22;line-height:1.8;">Your payment of <strong>AUD $${amountAud}</strong> has been received and your booking is confirmed. I am absolutely can't wait to work with you and make you feel stunning! ✨</p>
          </td>
        </tr>
